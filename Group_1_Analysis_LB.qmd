...
```{r}
#| label: fig-boxplot6
#| fig-subcap: Number of bedrooms in the house by Number of family members.
ggplot(data = FIES, aes(x = Total.Number.of.Family.members, y = Number.of.bedrooms, fill = Total.Number.of.Family.members)) + 
  geom_boxplot() + 
  labs(x = "Number of people living in the house.", y = "Number of bedrooms in the house") + 
  theme(legend.position = "none")
```
...
Overall, according to the separate analysis of explanatory variables, the variables that should be retained for now are:Annual household income, Annual expenditure by the household on food, Head of the households sex, Relationship between the group of people living in the house, Number of bedrooms in the house and Electricity.
...
model.FIES <- glm(Total.Number.of.Family.members ~ Total.Household.Income + Total.Food.Expenditure + Household.Head.Sex + Type.of.Household + Number.of.bedrooms + Electricity, data = FIES, family = poisson(link = "log"))

model.FIES %>%
  summary()
```

In examining the factors that affect the number of people living in a household, we observe that the coefficient for households headed by a male (Household.Head.SexMale) is positive, indicating a higher number of family members in households with male heads compared to female heads. The coefficients for households identified as Single Family (Type.of.HouseholdSingle Family) and for those with Two or More Nonrelated Persons/Members are negative, reflecting a lower count of family members in comparison with other types of households. Notably, the negative coefficient for single-family households is more substantial than for households with nonrelated members, suggesting that the expected number of people is significantly lower in single-family settings. Additionally, the coefficient for the Number of bedrooms is negative, suggesting that as the number of bedrooms in a house increases, the count of family members slightly decreases. This may indicate that with more available space, family members tend to spread out, or it could reflect a preference for smaller families in larger homes. The effects of Total Household Income and Total Food Expenditure are in opposite directions, with the former showing a slight decrease and the latter a slight increase in the count of family members with higher expenditures, implying that financial factors have nuanced impacts on household size. Each of these effects is statistically significant, pointing to a complex interplay of gender, household composition, and economic factors in determining family size.
